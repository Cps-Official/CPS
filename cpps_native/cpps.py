...
from ir import lower as lower_ir
from optimizer import PassManager
from native import NativeCompileError, compile_native
from package import PackageError, PackageResolver, load_manifest
...
""",
    ...
    mode_group.add_argument(
        '--check',
        action='store_true',
        help='Phân tích tĩnh (type + ownership check) mà không chạy chương trình'
    )
    mode_group.add_argument(
        '--type',
        action='store_true',
        help='Alias rõ nghĩa cho --check'
    )
    mode_group.add_argument(
        '--lint',
        action='store_true',
        help='Chạy semantic linter và hiển thị cảnh báo'
    )
    mode_group.add_argument(
        '--ir',
        action='store_true',
        help='Hạ AST xuống typed IR, verify và chạy optimizer'
    )
    mode_group.add_argument(
        '--build',
        action='store_true',
        help='Biên dịch native bằng GCC, không chạy interpreter'
    )
    mode_group.add_argument(
        '--run-native',
        action='store_true',
        help='Biên dịch native rồi chạy executable đã tạo'
    )
    mode_group.add_argument(
        '--format',
        action='store_true',
        help='Chuẩn hóa whitespace của source và in ra stdout'
    )
    mode_group.add_argument(
        '--test',
        action='store_true',
        help='Chạy test suite của project'
    )
    mode_group.add_argument(
        '--package',
        action='store_true',
        help='Resolve cp.toml dependencies và in lock plan'
    )
    mode_group.add_argument(
        '--benchmark',
        nargs='?',
        ...
        type=int,
        metavar='N',
        help='Chạy chương trình N lần và báo thời gian (mặc định 10)'
    )
    ...
    opt_group.add_argument(
        '--output', '-o',
        metavar='PATH',
        help='Đường dẫn executable cho --build/--run-native'
    )
    opt_group.add_argument(
        '--json',
        action='store_true',
        help='Xuất diagnostics ở định dạng JSON'
    )
...
def main() -> int:
    ...
    if args.package:
        manifest_path = os.path.join(os.getcwd(), 'cp.toml')
        try:
            manifest = load_manifest(manifest_path)
            lock = PackageResolver([os.path.dirname(manifest_path)]).resolve(manifest)
            print(lock.dumps(), end='')
            return 0
    ...
    if args.format:
        # Formatting is intentionally conservative: comments, literals and
        # operator spelling remain untouched; only trailing whitespace,
        # indentation tabs and blank-line runs are normalized.
    ...
    try:
        ...
        # Compiler pipeline modes are deliberately separate from execution.
        # They share the exact lexer/parser/AST produced above, then either
        # inspect IR or produce a real native executable.
        if args.ir:
            try:
                module = lower_ir(ast, os.path.basename(filepath))
                reports = PassManager().run(module)
                print(module.format())
                for report in reports:
                    print(
                        f"; pass {report.name}: changed={report.changed} "
        ...
        if args.build or args.run_native:
            result = semantic_analyze(ast, filepath, source)
            if result.has_errors:
                print(result.render(filepath, source), file=sys.stderr)
                return 1
            output = args.output
            if not output:
                stem = filepath[:-5] if filepath.endswith('.cpps') else filepath
                output = stem + '.native'
            try:
                native_result = compile_native(ast, output)
                print(f"✅ native executable: {native_result.executable}")
                if args.run_native:
                    completed = subprocess.run([native_result.executable])
                    return completed.returncode
                return 0
        ...
        # Static semantic analysis: type checking + ownership/borrow checking
        # (cpps_native/src/semantic.py). `cpps --check FILE` runs this alone
        # and exits without executing anything. During a normal run,
        # diagnostics are surfaced as warnings but don't block execution —
        # the checker is new and intentionally conservative, but until it
        # has run against a much wider body of real CP+* programs it stays
        # advisory outside of --check rather than silently changing what
        # already-working programs do.
        if args.check or args.type or args.lint:
            result = semantic_analyze(ast, filepath, source)
            if result.diagnostics:
                if args.json:
                    print(result.to_json(filepath))
                else:
                    print(result.render(filepath, source), file=sys.stderr)
                print(file=sys.stderr)
            if result.has_errors:
                n = sum(1 for d in result.diagnostics if d.severity.value == 'error')
                print(f"❌ check failed: {n} error(s)", file=sys.stderr)
                return 1
            if args.json:
                print(result.to_json(filepath))
            else:
                print(f"✅ {filepath}: no errors found")
            return 0
        else:
            result = semantic_analyze(ast, filepath, source)
            if result.diagnostics:
                print(f"\n⚠️  semantic analysis found {len(result.diagnostics)} issue(s) (run with --check for details):", file=sys.stderr)
                for d in result.diagnostics[:5]:
                    print(f"  [{d.code}] line {d.line}: {d.message}", file=sys.stderr)

        if args.benchmark is not None:
            iterations = max(1, args.benchmark)
            start = time.perf_counter()
            exit_code = 0
            for _ in range(iterations):
                exit_code = Interpreter(
                    verbose=False, max_depth=args.max_depth
                ).execute(ast)
                if exit_code:
                    break
            ...
            benchmark_result = {
                ...
                "completed": iterations if exit_code == 0 else _ + 1,
                "elapsed_ms": round(elapsed * 1000, 3),
                "mean_ms": round(elapsed * 1000 / max(1, iterations), 3),
                "exit_code": exit_code,
            }
            print(json.dumps(benchmark_result) if args.json else (
                f"benchmark: {iterations} iterations, "
                f"{elapsed * 1000:.3f}ms total, "
                f"{elapsed * 1000 / iterations:.3f}ms mean"
            ))
            return exit_code

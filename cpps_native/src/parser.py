...
class Pattern(ASTNode):
    ...
    A pattern is an AST node in its own right so visitors and diagnostics can
    traverse it uniformly even when a future pattern introduces no child
    expressions.
    """
...
class Parser:
    ...
    def parse_stmt(self) -> Optional[ASTNode]:
        ...
        # loop { ... }  — infinite loop, sugar for `while true { ... }`
        if t.type == TokenType.KW_LOOP:
        ...
        # NOTE: `Type::method(args)` (a static call used as a bare
        # statement, e.g. `Logger::warn("x")`) also starts with
        # `IDENTIFIER DOUBLE_COLON`, so we must not blindly treat every
        # such prefix as a var decl — check the lookahead first.
        if t.type == TokenType.IDENTIFIER:
            nxt = self.peek(1)
            if nxt and nxt.type == TokenType.KW_LET:
                return self.parse_var_decl()
            if nxt and nxt.type == TokenType.DOUBLE_COLON and not self._looks_like_static_call_stmt():
                return self.parse_var_decl()
    ...
    def _looks_like_static_call_stmt(self) -> bool:
        """
        Lookahead used at statement level to tell apart
        `name :: [mut] Type = expr` (a var decl) from
        `Type::method(args)` / `a::b::c(args)` used as a bare
        expression statement. Assumes the current token is an
        IDENTIFIER and `peek(1)` is DOUBLE_COLON. Non-consuming.
        """
        i = 2  # token right after '::'
        after_colon = self.peek(i)
        if after_colon is None or after_colon.type == TokenType.KW_MUT:
            return False
        if after_colon.type != TokenType.IDENTIFIER:
            return False
        i += 1
        while self.peek(i) and self.peek(i).type == TokenType.DOUBLE_COLON:
            i += 1
            nxt = self.peek(i)
            if nxt and nxt.type == TokenType.IDENTIFIER:
                i += 1
                ...
                break
        following = self.peek(i)
        return bool(following and following.type == TokenType.LPAREN)
    ...
    def parse_loop(self) -> WhileStmt:
        ...
        Desugars to `WhileStmt(Literal(True), body)` — there is no
        separate LoopStmt AST/runtime node, so `loop { }` gets break/
        continue handling and interpreter support for free from the
        existing `while` implementation. Only a bare `{ }` body is
        accepted (no `**` marker and no condition), matching how
        `loop` reads in every other language that has it.
        """
        line = self.peek().line
        self.advance()  # loop
        self.expect(TokenType.LBRACE)
        ...
        body = self.parse_block()
        self._loop_depth -= 1
        return WhileStmt(Literal(True, line), body, line)
    ...
    def parse_pattern(self) -> Pattern:
        """
        Parse a full pattern, including OR-alternatives: `p1 | p2 | p3`.
        Delegates the actual per-alternative parsing to
        `_parse_pattern_alt`.
        """
        line = self.peek().line if self.peek() else 0
        first = self._parse_pattern_alt()
        if self.check(TokenType.PIPE):
            alts = [first]
            while self.match(TokenType.PIPE):
                alts.append(self._parse_pattern_alt())
            return OrPattern(alts, line)
        return first

    def _parse_pattern_alt(self) -> Pattern:
        """
    ...
    def parse_compare(self) -> Optional[ASTNode]:
        """Parse comparison: expr == expr, expr != expr, expr is expr, etc."""
        ...
        while self.check(*compare_types) or self.check(TokenType.KW_IS):
            op = self.advance()
            right = self.parse_add()
            # `is` is CP+*'s value-comparison keyword (most commonly
            # `value is none`); it maps onto the same runtime equality
            # check as `==` rather than a separate identity concept,
            # since CP+* doesn't otherwise expose object identity.
            op_str = '==' if op.type == TokenType.KW_IS else op.value
            left = BinaryOp(left, op_str, right, op.line)
        return left

    def parse_range(self) -> Optional[ASTNode]:
        """
        Parse range expression: `start..end` (exclusive) or
        `start..=end` (inclusive).

        Binds looser than `+`/`-` (so `a+1..b-1` parses as
        `(a+1)..(b-1)`) and tighter than comparisons. Produces a
        `RangeExpr` node — the interpreter already evaluates this into
        a materialized list of ints, so it can be used anywhere an
        iterable is expected, most commonly in a for-each loop:
        `<> i :: 0..10 ** { ... }`. Not chainable (`a..b..c` is a
        parse error, same as every other language with ranges).
        """
        left = self.parse_add()
        if self.check(TokenType.DOTDOT) or self.check(TokenType.DOTDOTEQ):
            inclusive = self.check(TokenType.DOTDOTEQ)
            op = self.advance()
            right = self.parse_add()
            return RangeExpr(left, right, inclusive, op.line)
        return left
    ...
    def parse_primary(self) -> Optional[ASTNode]:
        ...
        # Lambda expression: |x, y| expr   or   |x, y| ** { block }
        # (also used as anonymous callbacks for :map/:filter/:reduce).
        # Safe to recognize `|` here because a primary/atom position can
        # never legitimately start with a binary bitwise-OR.
        if t.type == TokenType.PIPE:
            ...
            if not self.check(TokenType.PIPE):
                while True:
                    ...
                    if self.check(TokenType.COLON):
                        self.advance()
                        ptype = self.parse_type_ann()
                    params.append((pname, ptype, None, None))
                    if not self.match(TokenType.COMMA):
                        break
            ...
            self.match(TokenType.POW)  # optional `**` before a block body

            if self.check(TokenType.LBRACE):
                self.advance()
                body = self.parse_block()
            else:
                body = [ReturnStmt(self.parse_expr(), t.line)]

            return LambdaExpr(params, return_type, body, t.line)

        # Super reference: super::method(args) or super::field
        if t.type == TokenType.KW_SUPER:
            self.advance()
            name = 'super'
            if self.check(TokenType.DOUBLE_COLON):
                parts = [name]
